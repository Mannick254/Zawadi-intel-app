...
request({uri: 'https://ip.jsontest.com/'}, function (error, response, body) {
...
request({uri: 'https://ip.jsontest.com/'}, (error: any, response: r.Response, body: any) => {
  console.log('error:', error); // Print the error if one occurred
  console.log('statusCode:', response && response.statusCode); // Print the response status code if a response was received
  console.log('body:', body); // Print the JSON.
});
...
```ts
request({uri:'https://service.com/upload', method:'POST', json: {key:'value'}}, function(err,httpResponse,body){ /* ... */ })
```
...
let defaultRequest = teenyRequest.defaults({timeout: 60000});
      defaultRequest({uri: 'https://ip.jsontest.com/'}, function (error, response, body) {
